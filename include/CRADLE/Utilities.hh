...
#include "CRADLE/CorrelationCoefficient.hh"

#define FERMI 0
#define GAMOW_TELLER 1
#define MIXED 2
...
namespace CRADLE
{
  ...
  namespace utilities
  {
    ...
    inline int FindBetaType(Particle *initState, Particle *finalState)
    {
      DecayManager &dm = DecayManager::GetInstance();
    ...
    inline double RandomFromDistribution(std::vector<std::vector<double>> &pd, double max)
    {
      double begin = pd[0][0];
    ...
    {
      double max = CalculateMax(pd);
      return RandomFromDistribution(pd, max);
    }
    ...
    { /////////// L0 Correction ---- L0 (voir specific, ajouter le sum)
                                      ...
                                      double gM)
    {
      ...
      d1f = std::log(lambdaOverM) - EULER_MASCHERONI_CONSTANT + 4. / 3. -
            ...
            3.0 / M_PI / std::sqrt(10.0) * lambdaOverM * (0.5 + EULER_MASCHERONI_CONSTANT + std::log(std::sqrt(10) / lambdaOverM));
    ...
    inline double AtomicExchangeCorrection(double W, int Z)
    { ///////////// ATOMIC EXCHNAGE -------- X (crée par SL 10/05/2023)
      double E = W - 1;
    ...
    inline double GetSpectrumHeight(int Z, int A, double Q, double E, bool advanced, int decayType)
    {
      ...
      if (advanced)
      {
        int betaSign = (int)((Z > 0) - (Z < 0)); // signed previously using final and init state
        Z = std::abs(Z);
        double cShape, cNS;
        DecayManager &dm = DecayManager::GetInstance();
    ...
    inline vector<double> GetParticleDirection(ublas::vector<double> &dir2, std::vector<double> &A)
    {
      ...
      double costheta = RandomFromDistribution(dist, W_max);
      double theta = std::acos(costheta);
      vector<double> perp = CrossProduct(dir2, RandomDirection());
      perp = NormaliseVector(perp);
      vector<double> dir = RotateAroundVector(dir2, perp, theta);
      return dir;
    }

    inline vector<double> GetParticleDirection(vector<double> &dir2, double beta)
    {
      dir2 = NormaliseVector(dir2);

      /// Analytic solution for W(theta)=1+beta*cos(theta) with beta = a*p/W
      if (std::abs(beta) > 1.)
      {
      ...
      {
        costheta = 2.0 * r - 1.0; // isotropic distribution
      }
      else
      {
        const double disc = (1.0 - beta) * (1.0 - beta) + 4.0 * beta * r;
        costheta = (-1.0 + std::sqrt(disc)) / (beta);
      }
      ////
    ...
    inline vector<double> GetParticleDirection(vector<double> &dir2, double alpha, double beta)
    {
      dir2 = NormaliseVector(dir2);
      ...
      if (std::abs(kappa) > 1.)
      {
        Error("Kappa value " + std::to_string(kappa) + " is out of physical range.");
      }
      ...
      {
        const double disc = (1.0 - kappa) * (1.0 - kappa) + 4.0 * kappa * r;
        costheta = (-1.0 + std::sqrt(disc)) / (kappa);
      }
    ...
    inline double GetAMEMass(std::string filename, int Z, int A)
    {
      ...
      {
        Warning("Cannot open AME data file " + filename + ". Returning 0 for mass.");
        return 0.;
      }
